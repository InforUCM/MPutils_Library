...
class Test_MPUtils_Library:
    ...
    def test_mpu_uigetfile_no_args(self, mock_askopenfilename):
        ...
        self.assertEqual(filename, 'testfile.txt') # Check file name
        self.assertEqual(dirname, 'C:/user/testfile.txt') # Check directory name

        # 3.- Verify that askopenfilename was called with default parameters
        mock_askopenfilename.assert_called_once_with(defaultextension="*.*", filetypes=[("All Files", "*.*")], title="Select a file to open", initialfile="")
